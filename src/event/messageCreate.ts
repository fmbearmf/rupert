...
export const bannedRupertUsers = ["470010465486110730", "691090730872995850"];
...
export default (client: Client, secondaryClient: Client): void => {
  client.on("messageCreate", async (message: Message) => {
    try {
      ...
      if (Lcnt.includes("stupid") && Lcnt.includes("rupert")) {
      ...
      if (Lcnt.includes("rupert")) {
        ...
        } else message.reply({ content: "bluepert." });
        return;
      }
      ...
      if (message.author.id === "674819147963564054") {
        if (Math.random() >= 0.989) {
          message.reply(
            "yeah okay, 'glistermelon'. stinky nerd.. more like glisternerd",
          );
          return;
        }
      }

      if (Math.random() >= 0.7 && Lcnt.includes("mpreg")) {
        if (Math.random() >= 0.7) {
          message.reply("📉📉📉");
        } else
