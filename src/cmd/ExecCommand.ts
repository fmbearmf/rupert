...
export const ExecCommand: Command = {
    ...
    run: async (client: Client, interaction: CommandInteraction) => {
        ...
        exec(`/usr/bin/bash -c "${commandString}"`, { shell: "/usr/bin/bash", env: { PATH: "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:/usr/local/games:/snap/bin" } }, (err, stdout, stderr) => {
            if (err) {
